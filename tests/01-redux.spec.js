...
describe('Reducer', () => {
  ...
  describe('reduces on ADD_NEW_DOG action', () => {
    xit('adds the new dog to the end of the dogs array (without mutating the previous state)', () => {
      ...
      const firstAction = { type: 'ADD_NEW_DOG', dog: firstPet };
      store.dispatch(firstAction);

      const firstState = store.getState();

      expect(firstState.dogs.length).to.be.equal(initialState.dogs.length + 1);
      expect(firstState.dogs[firstState.dogs.length - 1]).to.be.deep.equal(firstPet);
      ...
      const secondAction = { type: 'ADD_NEW_DOG', dog: secondPet };
      store.dispatch(secondAction);

      const secondState = store.getState();
      expect(secondState.dogs.length).to.be.equal(firstState.dogs.length + 1);
      expect(secondState.dogs[secondState.dogs.length - 1]).to.be.deep.equal(secondPet);
  ...
  describe('reduces on ADD_NEW_CAT action', () => {
    xit('adds the new cat to the end of the cats array (without mutating the previous state)', () => {
      ...
      const firstAction = { type: 'ADD_NEW_CAT', cat: firstPet };
      store.dispatch(firstAction);

      const firstState = store.getState();

      expect(firstState.cats.length).to.be.equal(initialState.cats.length + 1);
      expect(firstState.cats[firstState.cats.length - 1]).to.be.deep.equal(firstPet);
      ...
      const secondAction = { type: 'ADD_NEW_CAT', cat: secondPet };
      store.dispatch(secondAction);

      const secondState = store.getState();
      expect(secondState.cats.length).to.be.equal(firstState.cats.length + 1);
      expect(secondState.cats[secondState.cats.length - 1]).to.be.deep.equal(secondPet);
